...
class SE_module(nn.Module):
    def __init__(self, in_planes):
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(in_planes, in_planes//16, 1, bias=False), # 首先降低维度
            nn.ReLU(inplace=True),
            nn.Conv2d(in_planes//16, in_planes, 1, bias=False), # 维度进行还原
            nn.Sigmoid(),
        )
...
class BasicBlock(nn.Module): # 残差模块建立 适用于 resnet18和resnet34，每个模块2个卷积层
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None):
    ...
    self.se = SE_module(planes) # 堆叠注意力模块
  ...
  def forward(self, x):
    ...
    out = self.se(out) * out  # SE注意力模块 element-wise multiplication
...
class Bottleneck(nn.Module): # 残差模块建立 适用于 resnet50、resnet101和resnet152，每个模块3个卷积层
  expansion = 4
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None):
    ...
    self.se = SE_module(planes * 4) # 堆叠注意力模块  planes * self.expansion
...
class PoseResNet(nn.Module):
  ...
  def _make_layer(self, block, planes, blocks, stride=1): # 大层。
    downsample = None
...
def get_pose_net_resnet_SE(num_layers, head_conv, num_classes=3):
...
if __name__ == '__main__':
  ...
  net = get_pose_net_resnet_SE(num_layers=18, head_conv=64)
