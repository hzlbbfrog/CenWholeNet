...
class BasicBlock(nn.Module): # 残差模块建立 适用于 resnet18和resnet34，每个模块2个卷积层
  ...
  def forward(self, x):
    ...
    out = self.ca(out) * out # 串联堆叠注意力模块 element-wise multiplication
    out = self.sa(out) * out # 串联堆叠注意力模块 element-wise multiplication
...
class Bottleneck(nn.Module): # 残差模块建立 适用于 resnet50、resnet101和resnet152，每个模块3个卷积层
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None):
    ...
    self.ca = ChannelAttention(planes * 4) # planes * self.expansion
    self.sa = SpatialAttention()
...
def get_pose_net_resnet_CBAM(num_layers, head_conv, num_classes=3):
...
if __name__ == '__main__':
  ...
  net = get_pose_net_resnet_CBAM(num_layers=18, head_conv=64)
